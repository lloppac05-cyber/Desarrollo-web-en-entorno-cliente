/*
 * @name canPlay
 * @description Devuelve si dada una mano de Uno y una carta en la mesa, puede jugar
 *  o tiene que retirar del mazo. Las cartas
 * tienen uno de 4 colores posibles (red, blue, green or yellow) y un número del 1 al 9.
 *  Una carta se escribe como un string
 * con color y número. Por ejemplo: "yellow 3", "red 6". Se puede jugar si la carta en 
 * la mano coincide en color o en número
 * con la carta en la mesa
 *
 * @param {string[]} hand - Array de cartas que tiene en la mano 
 * @param {string} face - Carta que está en la mesa
 * @returns {Boolean} - Devuelve true si puede jugar alguna carta, o false si tiene que 
 * retirar del mazo
 * 
 * @example
...
function canPlay(hand, face){
    let mesa = face.split(" ");
    let mano = hand.flatMap(carta => carta.split(" "));
    let jugar = false;
    for(let i = 0; i < mano.length && jugar == false; i++){
        for(let j = 0; j < mesa.length && jugar == false; j++){
            if(mano[i] == mesa[j]){
            jugar = true;
            }
        }
